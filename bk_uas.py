...
# Mounting google drive agar dapat mengakses file yang berada di google drive
from google.colab import drive
drive.mount('/content/drive')

# Menempatkan alamat pada directory data yang akan di gunakan
...
# Data raw dibaca menggunakan itertools agar dapat menjadi sebuah data sesuai fitur
lines = line
...
# Mengganti data dengan nilai -9 dengan nilai kosong atau NULL
df.replace('-9', np.nan, inplace=True)
df.head()

# Ubah data dengan fitur yang ingin digunakan
selected_columns = {
    2: 'age',
    3: 'sex',
    8: 'cp',
...
#Replace data dengan nilai NULL
...
# Mencari kolom dengan data NULL
df.loc[:, df.isnull().any()].columns

# Mengganti nilai NULL dengan nilai median dari kolom
...
# Cek apakah ada data dengan nilai NULL
df.isnull().values.any()

# Info dataset
df.info()

# Mebuat Confusion matrix
corr_matrix = df.corr()
...
# Plot dataset menjadi Target dan Fitur
y = df['num']
...
# Cek the shape of X and y before applying SMOTE
...
# Membuat Bar Plot dari data target sebelum dan sesduah dilakukan over sampling dengaan SMOTE
...
# Mengunggah model yang sudah ditrain
with open('rfModel.pkl', 'rb') as file:
    model = pickle.load(file)

# Membuat fungsi yang melakukan prediksi
